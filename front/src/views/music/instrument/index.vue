...
<div class="search">
            ...
            <Form ref="searchForm" :model="searchForm" inline :label-width="0">
                <Form-item ref="searchForm" :model="searchForm" inline :label-width="0" style="display:flex;">
                    <Form-item label="" prop="title">
                        <Input type="text" v-model="searchForm.title" placeholder="乐器名称" clearable style="width: 150px" />
                    </Form-item>
    ...
    <Modal v-model="timeModal" title="租赁时间选择" :mask="false" draggable width="600" footer-hide>
        <Row :gutter="16">
            <Col span="24">
            <DatePicker type="datetime" format="yyyy-MM-dd HH:mm" @on-change="changeSelectTime" style="width: 100%"></DatePicker>
            </Col>
            <Col span="24">
            <Divider>
                <Button @click="addOrderOnlineFx" type="success">确认租赁</Button>
            </Divider>
            </Col>
        </Row>
    </Modal>
...
export default {
                        ...
                        return h("div", [
                            h(
                                "Button", {
                                    ...
                                    on: {
                                        click: () => {
                                            this.timeObj = params.row;
                                            this.timeModal = true;
                                            // this.addOrderOnlineFx(params.row);
                                        }
                                    }
                                },
                                "租赁"
                            ...
                            //         on: {
                            //             click: () => {
                            //                 this.addOrderOfflineFx(params.row);
                            //             }
                            //         }
                            //     },
                            //     "线下租赁"
                            ...
                            h(
                                "Button", {
                                    ...
                                    style: {
                                        marginRight: "5px",
                                        "display": !(that.$route.meta.permTypes && that.$route.meta.permTypes.includes("edit")) ? "none" : ""
                                    },
                                    on: {
                                        click: () => {
    ...
    methods: {
        ...
        getDataList() {
            this.loading = true;
            getInstrumentList(this.searchForm).then(res => {
                this.loading = false;
                if (res.success) {
        ...
        addOrderOnlineFx() {
            ...
            this.$Modal.confirm({
                ...
                onOk: () => {
                    addOrderOnline({
                        id: that.timeObj.id,
                        time: that.selectTime
                    }).then(res => {
                        this.$Modal.remove();
                        if (res.success) {
                            that.timeModal = false;
